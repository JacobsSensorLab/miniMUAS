#include "./ServiceUser_WUAS.hpp"

NDN_LOG_INIT(muas.ServiceUser_WUAS);

muas::ServiceUser_WUAS::ServiceUser_WUAS(ndn::Face& face, ndn::Name group_prefix, ndn::security::Certificate identityCert,ndn::security::Certificate attrAuthorityCertificate, std::string trustSchemaPath)
...
muas::ServiceUser_WUAS::~ServiceUser_WUAS() {}

void muas::ServiceUser_WUAS::OnResponse(const ndn::svs::SVSPubSub::SubscriptionData &subscription)
{
        ...
        if (ServiceName.equals(m_EntityServiceStub.serviceName))
        {
            ...
            if(subscription.data.size() > 0){
                nacConsumer.consume(subscription.name,
                                    ...
                                    std::bind(&ServiceUser_WUAS::OnResponseDecryptionErrorCallback, this, providerName, ServiceName, FunctionName, RequestId, _1));
            }else{
                nacConsumer.consume(subscription.name,
                                    std::bind(&muas::EntityServiceStub::OnResponseDecryptionSuccessCallback, &m_EntityServiceStub, providerName, ServiceName, FunctionName, RequestId, _1),
                                    std::bind(&ServiceUser_WUAS::OnResponseDecryptionErrorCallback, this, providerName, ServiceName, FunctionName, RequestId, _1));
