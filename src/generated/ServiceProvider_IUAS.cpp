#include "./ServiceProvider_IUAS.hpp"

namespace muas
{
    NDN_LOG_INIT(muas.ServiceProvider_IUAS);
    ServiceProvider_IUAS::ServiceProvider_IUAS(ndn::Face& face, ndn::Name group_prefix, ndn::security::Certificate identityCert, ndn::security::Certificate attrAuthorityCertificate, std::string trustSchemaPath)
        : ndn_service_framework::ServiceProvider(face, group_prefix, identityCert, attrAuthorityCertificate,  trustSchemaPath),
        m_EntityService(*this),m_IUASService(*this),m_MissionService(*this),m_FlightCtrlService(*this),m_MAVLinkService(*this),m_SensorService(*this)
    {
    ...
    ServiceProvider_IUAS::~ServiceProvider_IUAS(){}

    void ServiceProvider_IUAS::registerServiceInfo()
    {
        ...
        details = {ndn::Name("/IUAS/PointOrbit"),
            identity,
            3600,
            time(NULL),
            { {"type", "IUAS"}, {"version", "1.0.0"}, {"tokenName", identity.toUri()+"/NDNSF/TOKEN/IUAS/PointOrbit/0"} }};
        m_ServiceDiscovery.publishServiceDetail(details);
        UpdateUPTWithServiceMetaInfo(details);
        
        

        details = {ndn::Name("/IUAS/PointHover"),
            identity,
            3600,
            time(NULL),
            { {"type", "IUAS"}, {"version", "1.0.0"}, {"tokenName", identity.toUri()+"/NDNSF/TOKEN/IUAS/PointHover/0"} }};
        m_ServiceDiscovery.publishServiceDetail(details);
        UpdateUPTWithServiceMetaInfo(details);
    ...
    void ServiceProvider_IUAS::ConsumeRequest(const ndn::Name& RequesterName,const ndn::Name& providerName,const ndn::Name& ServiceName,const ndn::Name& FunctionName, const ndn::Name& RequestID, ndn_service_framework::RequestMessage& requestMessage)
    {
        // log the request
        ...
        if (ServiceName.equals(m_IUASService.serviceName))
        {
            m_IUASService.ConsumeRequest(RequesterName, providerName, ServiceName, FunctionName, RequestID, requestMessage);                                  
        }
