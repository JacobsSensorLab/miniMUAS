...
main(int argc, char **argv)
{
    ...
    snprintf(sensor_namespace, sizeof(sensor_namespace), "/muas/%s/sensor/0", identity);
    ...
    auto takeoff = [&]() {
        ...
        providers.push_back(ndn::Name("/muas/iuas1"));
    ...
    try {
        while (1) {
            if (telemetry.in_air() && !single_request_sent) {
